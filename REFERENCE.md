# STS2 原版资料备查

> 本文件为杀戮尖塔2（Slay the Spire 2）原版数据的参考资料备份，所有数据保持原样，未做任何游戏转化。
>
> 数据来源：wiki.gg、mobalytics 等公开资料。
>
> 注意：部分遗物名称缺失（wiki 过滤器未捕获），效果描述中 `[energy]` / `[X]` 为原版能量符号。

---

## 一、故障机器人（Defect）卡牌

### 1.1 攻击卡（Attack Cards）

| # | 名称 | 原名 | 费用 | 稀有度 | STS2 原始效果 |
|---|------|------|------|--------|---------------|
| 1 | 爪击 | Claw | 0 | Common | 造成3伤害。增加此战斗中所有爪击卡2伤害。 |
| 2 | 球状闪电 | Ball Lightning | 1 | Common | 引导1闪电。 |
| 3 | 弹幕 | Barrage | 1 | Common | 对随机敌人造成7伤害，共2次。 |
| 4 | 光束单元 | Beam Cell | 1 | Common | 造成3伤害。施加1易伤。 |
| 5 | 编译驱动 | Compile Driver | 1 | Common | 造成7伤害。你拥有的每种独特球抽1张牌。 |
| 6 | 集中打击 | Focused Strike | 1 | Common | 造成9伤害。本回合获得1集中。 |
| 7 | 瞄准双眼 | Go for the Eyes | 1 | Common | 造成3伤害。若敌人意图攻击，施加1虚弱。 |
| 8 | 污秽攻击 | Gunk Up | 1 | Common | 造成4伤害，共3次。将1张黏液加入你的弃牌堆。 |
| 9 | 引雷针 | Lightning Rod | 1 | Common | （冰霜/闪电相关） |
| 10 | 扫射光束 | Sweeping Beam | 1 | Common | 对所有敌人造成6伤害。抽1张牌。 |
| 11 | 动能打击 | Momentum Strike | 1 | Common | 造成10伤害。此卡费用降为0。 |
| 12 | 冷头 | Cold Snap | 1 | Common | （冰球相关，被评为最强冰球卡） |
| 13 | 冷静 | Coolheaded | 1 | Common | （冰霜相关） |
| 14 | 骚动 | Uproar | 2 | Uncommon | 造成5伤害，共2次。从抽牌堆随机打出1张攻击牌。 |
| 15 | 空无 | Null | 2 | Uncommon | 造成10伤害。施加2虚弱。引导1暗。 |
| 16 | 折射 | Refract | 3 | Uncommon | 造成9伤害，共2次。引导2玻璃。 |
| 17 | 刮削 | Scrape | 0 | Uncommon | 造成7伤害。抽4张牌。丢弃以此方式抽到的所有非0费牌。 |
| 18 | 特斯拉线圈 | Tesla Coil | 1 | Uncommon | 造成3伤害。对所有敌人触发所有闪电。 |
| 19 | 适应打击 | Adaptive Strike | 2 | Uncommon | 造成18伤害。将此卡的1张0费复制加入弃牌堆。 |
| 20 | 万物一心 | All for One | 1 | Uncommon | 造成10伤害。将弃牌堆中所有0费牌放入手中。 |
| 21 | 散射炮 | Flak Cannon | 1 | Rare | 消耗所有状态牌。每消耗1张，对随机敌人造成8伤害。 |
| 22 | 螺旋钻机 | Helix Drill | X | Rare | 造成3伤害，乘以本回合之前花费的能量数。 |
| 23 | 超能光束 | Hyperbeam | 3 | Rare | 对所有敌人造成26伤害。失去3集中。 |
| 24 | （未命名） | - | 1 | Uncommon | 造成13伤害。抽1张牌。当创建状态牌时，此卡本回合费用降为0。 |
| 25 | （未命名） | - | 1 | Common | 造成7伤害，对随机敌人共2次。 |

### 1.2 技能卡（Skill Cards）

| # | 名称 | 原名 | 费用 | 稀有度 | STS2 原始效果 |
|---|------|------|------|--------|---------------|
| 1 | 充电电池 | Charge Battery | 1 | Common | 获得7格挡。下回合获得[能量]。 |
| 2 | 全息影像 | Hologram | 1 | Common | 获得3格挡。将弃牌堆中的1张牌放入手中。消耗。 |
| 3 | 涡轮 | TURBO | 0 | Common | 获得[能量]。将1张虚空加入你的弃牌堆。 |
| 4 | 电容器 | Capacitor | 1 | Common | 获得2球槽。 |
| 5 | 黑暗 | Darkness | 1 | Common | 引导1暗。触发所有暗的被动能力。 |
| 6 | 超光速 | FTL | 1 | Common | 造成5伤害。若本回合打出的牌少于3张，抽1张牌。 |
| 7 | 野性 | Feral | 1 | Common | 本回合打出的第1张0费攻击返回手中。 |
| 8 | 冰雹风暴 | Hailstorm | 1 | Common | 回合结束时，若你有冰霜，对所有敌人造成6伤害。 |
| 9 | 迭代 | Iteration | 0 | Common | 每回合首次抽到状态牌时，额外抽2张牌。 |
| 10 | 循环 | Loop | 1 | Common | 回合开始时，触发最右侧球的被动能力。 |
| 11 | 超频 | Overclock | 1 | Common | 抽2张牌。将1张灼伤加入弃牌堆。 |
| 12 | 浏览 | Skim | 1 | Common | 抽3张牌。 |
| 13 | 烟囱 | Smokestack | 1 | Common | 每当你创建状态牌时，对所有敌人造成5伤害。 |
| 14 | 同步 | Synchronize | 0 | Uncommon | 本回合获得2集中，乘以你拥有的独特球数。消耗。 |
| 15 | 人工合成 | Synthesis | 1 | Uncommon | 造成12伤害。你打出的下一张能力牌费用为0。 |
| 16 | 白噪音 | White Noise | 0 | Uncommon | 将1张随机能力牌加入手中。本回合免费打出。消耗。 |
| 17 | 高速脱离 | Boost Away | ? | Uncommon | （防御/位移相关） |
| 18 | 暗影之盾 | Shadow Shield | ? | Uncommon | 大号铁斩波。 |
| 19 | 寒冷 | Chill | ? | Common | （冰霜相关） |
| 20 | 冰川 | Glacier | ? | Uncommon | （冰霜/格挡相关） |
| 21 | 拾荒 | Scavenge | ? | Common | （回收相关） |
| 22 | 压缩 | Compact | 1 | Rare | （描述：如此强大，居然只需要一点法力值） |
| 23 | 玻璃工艺 | Glasswork | ? | Common | 过渡牌，后期跟不上。 |
| 24 | 风暴 | Storm | ? | Rare | （冰霜/闪电相关） |
| 25 | 雷霆 | Thunder | ? | Rare | 配合电球数量增强。 |
| 26 | 电流相生 | Conduit | ? | Rare | 超模牌，打出后不消耗，一张牌就能做输出终端。 |
| 27 | 强撑 | Fight Through | ? | Common | （防御相关，偷自战士） |

### 1.3 能力卡（Power Cards）

| # | 名称 | 原名 | 费用 | 稀有度 | STS2 原始效果 |
|---|------|------|------|--------|---------------|
| 1 | 缓冲 | Buffer | 1 | Common | 防止下一次你会失去HP。 |
| 2 | 吞噬暗影 | Consuming Shadow | 2 | Uncommon | 引导2暗。回合结束时激发最左侧的球。 |
| 3 | 冷却剂 | Coolant | 1 | Uncommon | 回合开始时，获得2格挡，乘以你拥有的独特球数。 |
| 4 | 创造性AI | Creative AI | 1 | Rare | 回合开始时，将1张随机能力牌加入手中。 |
| 5 | 回声形态 | Echo Form | 3 | Rare | 虚无。你每回合打出的第1张牌额外打出1次。 |
| 6 | 机器学习 | Machine Learning | 1 | Common | 回合开始时，额外抽1张牌。 |
| 7 | 模组改造 | Modded | 1 | Common | 获得1球槽。抽1张牌。此卡费用增加1。 |
| 8 | 多重施放 | Multi-Cast | X | Rare | 激发最右侧的球X次。 |
| 9 | 重启 | Reboot | 2 | Uncommon | 将所有牌洗入抽牌堆。抽4张牌。消耗。 |
| 10 | 打碎 | Shatter | 2 | Rare | 对所有敌人造成11伤害。激发所有球。 |
| 11 | 信号增强 | Signal Boost | 1 | Uncommon | 你打出的下一张能力牌额外打出1次。消耗。 |
| 12 | 旋转工艺 | Spinner | 1 | Common | 回合开始时，引导1玻璃。 |
| 13 | 化废为宝 | Trash to Treasure | 1 | Rare | 每当你创建状态牌时，引导1个随机球。 |
| 14 | 偏差认知 | Biased Cognition | 1 | Rare | 获得4集中。回合开始时失去1集中。 |
| 15 | 放大 | Amplify | 1 | Uncommon | 激发最右侧的球4次。 |
| 16 | 启动 | Boot Up | 1 | Common | 回合开始时，将1张随机普通牌加入手中。 |
| 17 | 碎片整理 | Defragment | ? | Rare | （整理/优化相关，金卡） |
| 18 | 遗传算法 | Genetic Algorithm | ? | Rare | 类似灵体（减伤效果）。 |
| 19 | 自修复 | Self Repair | 1 | Common | 获得等于弃牌堆牌数的格挡。 |
| 20 | 自动化 | Automation | 0 | Uncommon | 每抽10张牌，获得[能量]。 |
| 21 | 递归 | Recursion | 1 | Uncommon | 造成9伤害。将你本回合打出的下一张牌放到抽牌堆顶。 |
| 22 | 热修复 | Hotfix | ? | Uncommon | 零费，临时的集中。 |
| 23 | 彩虹 | Rainbow | ? | Rare | （多球体系相关） |
| 24 | 超临界态 | Supercritical | ? | Rare | （与状态牌相关，风险较高） |
| 25 | 点火 | Ignition | ? | Rare | （燃烧/激发相关） |
| 26 | 冰之长枪 | Ice Lance | ? | Rare | （高数值冰霜卡） |

### 1.4 充能球（Orbs）

| # | 名称 | 原名 | 被动效果 | 激发效果 |
|---|------|------|----------|----------|
| 1 | 闪电 | Lightning | 被动触发时对随机敌人造成8伤害 | 激发时对所有敌人造成10伤害 |
| 2 | 冰霜 | Frost | 被动触发时获得2格挡 | 激发时对所有敌人造成5伤害 |
| 3 | 暗影 | Dark | 被动触发时积累伤害（每点+2，上限10） | 激发时释放积累的全部伤害给随机敌人 |
| 4 | 玻璃 | Glass | 被动触发时对最近敌人造成6伤害 | 激发时对所有敌人造成13伤害 |

### 1.5 状态牌（Status Cards）

| 名称 | 原名 | 效果 |
|------|------|------|
| 虚空 | Void | 手牌中时，获得1[能量]。回合结束时，受到2伤害。不可升级。 |
| 灼伤 | Burn | 回合结束时，受到6伤害。不可升级。 |
| 黏液 | Slimed | 不可打出。回合结束时，受到2伤害。不可升级。 |

---

## 二、遗物（Relics）

> 数据来源：wiki.gg Slay the Spire 2 Relics List。部分遗物名称缺失（wiki 过滤器未捕获），部分效果未知。
>
> 注意：部分效果描述中 `[energy]` / `[X]` 为原版能量符号，`???` 后缀表示降级/古代版本。

### 2.1 初始遗物（Starter Relics）

| 角色 | 遗物名称 | 效果 |
|------|----------|------|
| Ironclad | Burning Blood | At the end of combat, heal 6 HP. |
| Ironclad | （Burning Blood 升级版） | At the end of combat, heal 12 HP. 仅在拥有 Burning Blood 时可获得，不可从 Boss Swap 获得。 |
| Silent | （未命名） | At the start of each combat, draw 2 additional cards. |
| Silent | （未命名） | At the start of your first 3 turns, draw 2 additional cards. |
| Regent | （未命名） | At the start of each combat, gain [energy]. |
| Regent | Divine Destiny | （效果未知） |
| Necrobinder | （未命名） | At the start of your turn, Summon 1. |
| Necrobinder | Phylactery Unbound | At the start of each combat, Summon 5. At the start of your turn, Summon 2. |
| Defect | （未命名） | （效果未知） |
| Defect | （未命名） | （效果未知） |

### 2.2 普通遗物（Common Relics）

| 遗物名称 | 效果 |
|----------|------|
| Amethyst Aubergine | Enemies drop 10 additional Gold. |
| Anchor | Start each combat with 10 Block. |
| Bag of Preparation | At the start of each combat, draw 2 additional cards. |
| Blood Vial | At the start of each combat, heal 2 HP. |
| Book of Five Rings | Every 5 cards you add to your Deck, heal 15 HP. |
| Bronze Scales | Start each combat with 3 Thorns. |
| Centennial Puzzle | The first time you lose HP each combat, draw 3 cards. |
| Festive Popper | At the start of each combat, deal 9 damage to ALL enemies. |
| Gorget | At the start of each combat, gain 4 Plating. |
| Happy Flower | Every 3 turns, gain [energy]. |
| Juzu Bracelet | Regular enemy combats are no longer encountered in ? rooms. |
| Lantern | Start each combat with an additional [energy]. |
| Meal Ticket | Whenever you enter a shop room, heal 15 HP. |
| Oddly Smooth Stone | （效果未知） |
| Pendulum | Whenever you shuffle your Draw Pile, draw a card. |
| Permafrost | The first time you play a Powers each combat, gain 6 Block. |
| Potion Belt | Upon pickup, gain 2 potion slots. |
| Regal Pillow | Whenever you Rest, heal an additional 15 HP. |
| Strawberry | Upon pickup, raise your Max HP by 7. |
| Strike Dummy | Cards containing "Strike" deal 3 additional damage. |
| Tiny Mailbox | Whenever you Rest, procure a random potion. |
| Vajra | Start each combat with 1 Strength. |
| Venerable Tea Set | Whenever you enter a Rest Site, start the next combat with an additional [energy]. |
| （未命名） | （效果未知） |
| Whetstone | （效果未知） |

### 2.3 角色限定普通遗物（Common - Character Specific）

| 遗物名称 | 角色 | 效果 |
|----------|------|------|
| Red Skull | Ironclad | While your HP is at or below 50%, you have 3 additional Strength. |
| Snecko Skull | Silent | Whenever you apply Poison, apply an additional 1 Poison. |
| Fencing Manual | Regent | At the start of each combat, Forge 10. |
| Bone Flute | Necrobinder | Whenever Osty attacks, gain 2 Block. |
| Data Disk | Defect | Start each combat with 1 Focus. |

### 2.4 稀有遗物（Uncommon Relics）

| 遗物名称 | 效果 |
|----------|------|
| Akabeko | At the start of each combat, gain 8 Vigor. |
| Bag of Marbles | At the start of each combat, apply 1 Vulnerable to ALL enemies. |
| Bellows | The first Hand you draw each combat is Upgraded. |
| Bowler Hat | Gain 20% additional Gold. |
| Candelabra | At the start of your 2nd turn, gain [energy]. |
| Eternal Feather | For every 5 cards in your Deck, heal 3 HP whenever you enter a Rest Site. |
| Gremlin Horn | Whenever an enemy dies, gain [energy] and draw 1 card. |
| Horn Cleat | At the start of your 2nd turn, gain 14 Block. |
| Joss Paper | Every 5 times you Exhaust a card, draw 1 card. |
| Kusarigama | Every time you play 3 Attacks in a single turn, deal 6 damage to a random enemy. |
| Letter Opener | Every time you play 3 Skills in a single turn, deal 5 damage to ALL enemies. |
| Lucky Fysh | Whenever you add a card to your Deck, gain 15 Gold. |
| Mercury Hourglass | At the start of your turn, deal 3 damage to ALL enemies. |
| Miniature Cannon | （效果未知） |
| Nunchaku | Every time you play 10 Attacks, gain [energy]. |
| Orichalcum | If you end your turn without Block, gain 6 Block. |
| Ornamental Fan | Every time you play 3 Attacks in a single turn, gain 4 Block. |
| Pantograph | At the start of each Boss combat, heal 25 HP. |
| Parrying Shield | If you end a turn with at least 10 Block, deal 6 damage to a random enemy. |
| Pear | Upon pickup, raise your Max HP by 10. |
| Pen Nib | Every 10th Attack you play deals double damage. |
| Petrified Toad | （效果未知） |
| Planisphere | Whenever you enter a ? room, heal 4 HP. |
| Red Mask | At the start of each combat, apply 1 Weak to ALL enemies. |
| Reptile Trinket | Whenever you use a potion, gain 3 Strength this turn. |
| Ripple Basin | If you did not play any Attacks during your turn, gain 4 Block. |
| Sparkling Rouge | （效果未知） |
| Stone Cracker | At the start of Boss combats, Upgrade 3 random cards in your Draw Pile for the rest of combat. |
| Tuning Fork | Every time you play 10 Skills, gain 7 Block. |
| Vambrace | The first time you gain Block from a card each combat, double the amount gained. |

### 2.5 角色限定稀有遗物（Uncommon - Character Specific）

| 遗物名称 | 角色 | 效果 |
|----------|------|------|
| Paper Phrog | Ironclad | Enemies with Vulnerable take 75% more damage rather than 50%. |
| Self-Forming Clay | Ironclad | Whenever you lose HP in combat, gain 3 Block next turn. |
| Tingsha | Silent | Whenever you discard a card during your turn, deal 3 damage to a random enemy for each card discarded. |
| Twisted Funnel | Silent | At the start of each combat, apply 4 Poison to ALL enemies. |
| Galactic Dust | Regent | For every 10 [gold] spent, gain 10 Block. |
| Regalite | Regent | Whenever you create a Colorless card, gain 2 Block. |
| Book Repair Knife | Necrobinder | Whenever a non-Minion enemy dies to Doom, heal 3 HP. |
| Funerary Mask | Necrobinder | At the start of each combat, add 3 Souls into your Draw Pile. |
| Gold-Plated Cables | Defect | Your rightmost Orb triggers its passive an additional time. |
| Symbiotic Virus | Defect | （效果未知） |

### 2.6 珍贵遗物（Rare Relics）

| 遗物名称 | 效果 |
|----------|------|
| Art of War | If you do not play any Attacks during your turn, gain an additional [energy] next turn. |
| Beating Remnant | You cannot lose more than 20 HP in a single turn. |
| Captain's Wheel | At the start of your 3rd turn, gain 18 Block. |
| Chandelier | At the start of your 3rd turn, gain [energy]. |
| Cloak Clasp | At the end of your turn, gain 1 Block for each card in your Hand. |
| Frozen Egg | Whenever you add a Powers into your Deck, Upgrade it. |
| Gambling Chip | At the start of each combat, discard any number of cards then draw that many. |
| Game Piece | Whenever you play a Power, draw 1 card. |
| Girya | You can now gain Strength at Rest Sites. (3 times max) |
| （未命名） | Energy is now conserved between turns. |
| Intimidating Helmet | Whenever you play a card that costs [X] or more, gain 4 Block. |
| Kunai | Every time you play 3 Attacks in a single turn, gain 1 Dexterity. |
| Lasting Candy | Every other combat, your card rewards gain an additional Power. |
| Lizard Tail | When you would die, heal to 50% of your Max HP instead (works once). |
| Mango | Upon pickup, raise your Max HP by 14. |
| Meat on the Bone | If your HP is at or below 50% at the end of combat, heal 12 HP. |
| Molten Egg | Whenever you add an Attack card to your Deck, Upgrade it. |
| Mummified Hand | Whenever you play a Power, a random card in your Hand is free to play that turn. |
| Old Coin | Upon pickup, gain 300 Gold. |
| Pocketwatch | Whenever you play 3 or fewer cards during your turn, draw 3 additional cards at the start of your next turn. |
| Prayer Wheel | Normal enemies drop an additional card reward. |
| Rainbow Ring | （效果未知） |
| Razor Tooth | Every time you play an Attack or Skill, Upgrade it for the remainder of combat. |
| Shovel | You can now dig at Rest Sites to obtain a random Relic. |
| Shuriken | Every time you play 3 Attacks in a single turn, gain 1 Strength. |
| Stone Calendar | At the end of turn 7, deal 52 damage to ALL enemies. |
| Sturdy Clamp | Up to 10 Block persists across turns. |
| The Courier | The merchant no longer runs out of cards, relics, or potions and his prices are reduced by 20%. |
| Toxic Egg | Whenever you add a Skill into your Deck, Upgrade it. |
| Tungsten Rod | Whenever you would lose HP, lose 1 less. |
| Unceasing Top | Whenever you have no cards in Hand during your turn, draw a card. |
| Unsettling Lamp | Each combat, the first time you play a card that Debuffs an enemy, double its effect. |
| Vexing Puzzlebox | At the start of each combat, add a random card into your Hand. It costs 0. |
| White Beast Statue | Potions always appear in combat rewards. |
| White Star | Elites drop an additional Rare card reward. |

### 2.7 角色限定珍贵遗物（Rare - Character Specific）

| 遗物名称 | 角色 | 效果 |
|----------|------|------|
| Charon's Ashes | Ironclad | Whenever you Exhaust a card, deal 3 damage to ALL enemies. |
| Demon Tongue | Ironclad | The first time you lose HP on your turn, heal HP equal to the amount lost. |
| Ruined Helmet | Ironclad | The first time you gain Strength each combat, double the amount gained. |
| Helical Dart | Silent | Whenever you play a Shiv, gain 1 Dexterity this turn. |
| Paper Krane | Silent | Enemies with Weak deal 40% less damage to you rather than 25%. |
| Tough Bandages | Silent | Whenever you discard a card during your turn, gain 3 Block. |
| Lunar Pastry | Regent | At the end of your turn, gain [energy]. |
| Mini Regent | Regent | The first time you spend [energy] each turn, gain 1 Strength. |
| Orange Dough | Regent | At the start of each combat, add 2 random Colorless cards into your Hand. |
| Big Hat | Necrobinder | At the start of each combat, add 2 random Ethereal cards into your Hand. |
| Bookmark | Necrobinder | At the end of each turn, lower the cost of a random Retained card by 1 until played. |
| Ivory Tile | Necrobinder | Whenever you play a card that costs [X] or more, gain [energy]. |
| Emotion Chip | Defect | If you lost HP during the previous turn, trigger the passive ability of all Orbs at the start of your turn. |
| Metronome | Defect | The first time you Channel 7 Orbs each combat, deal 30 damage to ALL enemies. |
| Power Cell | Defect | At the start of each combat, add 2 zero-cost cards from your Draw Pile into your Hand. |

### 2.8 Boss 遗物（Boss Relics）

| 遗物名称 | 效果 |
|----------|------|
| （未命名） | Upon pickup, obtain a random Rare Card to add to your Deck. |
| Booming Conch | At the start of Elite combats, draw 2 additional cards. |
| Cursed Pearl | Upon pickup, receive Greed. Gain 333 Gold. |
| Golden Pearl | Upon pickup, gain 150 Gold. |
| Large Capsule | Upon pickup, obtain 2 random Relics. Add an additional Strike and Defend to your Deck. |
| Lava Rock | The Act 1 Boss drops 2 Relics. |
| Lead Paperweight | Upon pickup, choose 1 of 2 Colorless cards to add to your Deck. |
| Leafy Poultice | Upon pickup, Transform 1 of your Strikes and 1 of your Defends and lose 10 Max HP. |
| Lost Coffer | Upon pickup, gain 1 card reward and procure 1 random potion. |
| Massive Scroll | Upon pickup, choose 1 of 3 Multiplayer Colorless Cards to add to your Deck. |
| Neow's Torment | Upon pickup, add 1 Neow's Fury to your Deck. |
| New Leaf | （效果未知） |
| Nutritious Oyster | Upon pickup, raise your Max HP by 11. |
| Pomander | （效果未知） |
| Precarious Shears | Upon pickup, remove 2 cards from your Deck and take 13 damage. |
| Precise Scissors | Upon pickup, remove 1 card from your Deck. |
| Scroll Boxes | Upon pickup, lose all Gold and choose 1 of 2 packs of cards to add to your Deck. |
| Silver Crucible | The first 3 card rewards you see are Upgraded. The first Treasure Chest you open is empty. |
| Small Capsule | Upon pickup, obtain a random Relic. |
| Stone Humidifier | Whenever you Rest at a Rest Site, raise your Max HP by 5. |
| （未命名） | Upon pickup, gain 4 potion slots filled with random potions. |

### 2.9 角色限定 Boss 遗物（Boss - Character Specific）

| 遗物名称 | 角色 | 效果 |
|----------|------|------|
| Brimstone | Ironclad | At the start of your turn, gain 2 Strength and ALL enemies gain 1 Strength. |
| Ninja Scroll | Silent | At the start of each combat, add 3 Shivs into your Hand. |
| Vitruvian Minion | Regent | Cards containing "Minion" deal double damage and gain double Block. |
| Undying Sigil | Necrobinder | Enemies with at least as much Doom as HP deal 50% less damage. |
| Runic Capacitor | Defect | Start each combat with 3 additional Orb Slots. |

### 2.10 高阶 Boss 遗物（Elite Boss Relics）

| 遗物名称 | 效果 |
|----------|------|
| Astrolabe | （效果未知） |
| Black Star | Elites drop an additional Relic when defeated. |
| Calling Bell | Upon pickup, obtain a unique Curse and 3 Relics. |
| Dusty Tome | Upon pickup, obtain an Ancient Card. |
| Ectoplasm | You can no longer gain Gold. Gain [energy] at the start of each turn. |
| Empty Cage | Upon pickup, remove 2 cards from your Deck. |
| Pandora's Box | Transform ALL Strikes and Defends. |
| Philosopher's Stone | Gain [energy] at the start of each turn. ALL enemies start combat with 1 Strength. |
| Runic Pyramid | At the end of your turn, you no longer discard your Hand. |
| Snecko Eye | At the start of your turn, draw 2 additional cards. Start each combat Confused. |
| Sozu | Gain [energy] at the start of each turn. You can no longer obtain potions. |
| Velvet Choker | Gain [energy] at the start of each turn. You cannot play more than 6 cards per turn. |

### 2.11 商店遗物（Shop Relics）

| 遗物名称 | 效果 |
|----------|------|
| Archaic Tooth | Upon pickup, Transform a starter card with an ancient version. |
| Driftwood | You may reroll each card reward once. |
| Electric Shrymp | （效果未知） |
| Glass Eye | Upon pickup, obtain 2 Common cards, 2 Uncommon cards, and 1 Rare card. |
| （未命名） | Gain [energy] at the start of each turn. Card rewards now contain cards from other colors. |
| Radiant Pearl | At the start of each combat, add 1 Luminesce into your Hand. |
| Sand Castle | Upon pickup, Upgrade 6 random cards. |
| （未命名） | See 15 cards from another character. Choose any number of them to add to your Deck. |
| Touch of Orobas | Upon pickup, replace your starter Relic with an Ancient version. |

### 2.12 无色遗物（Colorless Relics）

| 遗物名称 | 效果 |
|----------|------|
| Beautiful Bracelet | Upon pickup, choose 3 cards in your Deck. Enchant them with Swift 3. |
| Blessed Antler | Gain [energy] at the start of each turn. At the start of each combat, shuffle 3 Dazed into your Draw Pile. |
| Brilliant Scarf | The 5th card you play each turn is free. |
| Belt Buckle | While you have no potions, you have 2 additional Dexterity. |
| Bread | At the start of your first turn, lose [energy]. At the start of all other turns, gain [energy]. |
| Burning Sticks | The first time each combat you Exhaust a Skill, add a copy of it into your Hand. |
| Cauldron | Upon pickup, brews 5 random potions. |
| Chemical X | The effects of your cost X cards are increased by 2. WARNING: Do not combine with sugar, spice, and everything nice. |
| Claws | Upon pickup, Transform up to 6 cards into Maul. |
| Crossbow | At the start of your turn, add a random Attack into your Hand. It costs 0 this turn. |
| Delicate Frond | At the start of each combat, fill all empty potion slots with random potions. |
| Diamond Diadem | Whenever you play 2 or fewer cards in a turn, take half damage from enemies. |
| Dingy Rug | Card rewards can now contain Colorless cards. |
| Dolly's Mirror | Upon pickup, obtain an additional copy of a card in your Deck. |
| Dragon Fruit | Whenever you gain Gold, raise your Max HP by 1. |
| Fur Coat | Upon pickup, mark 7 random combats. Enemies in those rooms have 1 HP. |
| Ghost Seed | Strikes and Defends gain Ethereal. |
| Gnarled Hammer | （效果未知） |
| Iron Club | Every 4 cards you play, draw 1 card. |
| Jewelry Box | Upon pickup, add 1 Apotheosis to your Deck. |
| Kifuda | （效果未知） |
| Lava Lamp | At the end of combat, Upgrade all card rewards if you took no damage. |
| Lee's Waffle | Upon pickup, raise your Max HP by 7 and heal all of your HP. |
| Looming Fruit | Upon pickup, raise your Max HP by 31. |
| Membership Card | 50% discount on all products! |
| Miniature Tent | You may choose any number of options at Rest Sites. |
| Mystic Lighter | （效果未知） |
| Orrery | Upon pickup, gain 5 card rewards. |
| Punch Dagger | （效果未知） |
| Ringing Triangle | Retain your Hand on the first turn of combat. |
| Royal Stamp | （效果未知） |
| Sai | At the start of your turn, gain 7 Block. |
| Screaming Flagon | If you end your turn with no cards in your Hand, deal 20 damage to ALL enemies. |
| Signet Ring | Upon pickup, gain 999 Gold. |
| Sling of Courage | Start each Elite combat with 2 Strength. |
| Spiked Gauntlets | Gain [energy] at the start of each turn. Powers cost 1 more [energy]. |
| Tanx's Whistle | Upon pickup, add 1 Whistle to your Deck. |
| Throwing Axe | The first card you play each combat is played an extra time. |
| Tri-Boomerang | （效果未知） |
| War Hammer | Whenever you kill an Elite, Upgrade 4 random cards. |
| The Abacus | Whenever you shuffle your Draw Pile, gain 6 Block. |
| Toolbox | At the start of each combat, choose 1 of 3 random Colorless cards and add the chosen card into your Hand. |
| Wing Charm | A random card in each card reward is Enchanted with Swift 1. |

### 2.13 特殊遗物（Special/Cursed Relics）

| 遗物名称 | 效果 |
|----------|------|
| Blood-Soaked Rose | Upon pickup, add 1 Enthralled to your Deck. Gain [energy] at the start of each turn. |
| Choices Paradox | At the start of each combat, add 1 of 5 random cards into your Hand. Add Retain to the chosen card. |
| Distinguished Cape | Upon pickup, lose 9 Max HP. Add 3 Apparitions to your Deck. |
| Fiddle | At the start of each turn, draw 2 additional cards. You may not draw cards during your turn. |
| Jeweled Mask | At the start of combat put a random Powers from your Draw Pile into your Hand, it's free to play. |
| Lord's Parasol | When you encounter the Merchant, immediately obtain EVERYTHING he sells. |
| Music Box | Create an Ethereal copy of the first Attack you play each turn. |
| Preserved Fog | Upon pickup, remove 5 cards from your Deck. Add Folly to your Deck. |
| Sere Talon | Upon pickup, add 2 random Curses and 3 Wishes to your Deck. |
| Whispering Earring | Gain [energy] at the start of each turn. Vakuu plays your first turn for you. |

### 2.14 Pael 遗物（Pael's Relics）

| 遗物名称 | 效果 |
|----------|------|
| Pael's Blood | At the start of your turn, draw 1 additional card. |
| Pael's Claw | （效果未知） |
| Pael's Eye | The first time each combat you end your turn without playing cards, Exhaust your Hand, and take an extra turn. |
| Pael's Flesh | Gain an additional [energy] at the start of your 3rd turn, and every turn after that. |
| （未命名） | （效果未知） |
| Pael's Horn | Upon pickup, add 2 Relax to your Deck. |
| Pael's Legion | Doubles Block gained from a card, then goes to sleep for 2 turns. |
| Pael's Tears | If you end your turn with unspent [energy], gain an additional [energy] next turn. |
| Pael's Tooth | Upon pickup, remove 5 cards from your Deck. After each combat, randomly add 1 back Upgraded. |
| Pael's Wing | You may sacrifice card rewards to Pael. Every 2 sacrifices, obtain a Relic. |

### 2.15 事件遗物（Event Relics）

| 遗物名称 | 效果 |
|----------|------|
| Biiig Hug | Upon pickup, remove 4 cards from your Deck. Whenever you shuffle your Draw Pile, add a Soot into your Draw Pile. |
| Golden Compass | Upon pickup, replace the Act 2 Map with a single special path. |
| Nutritious Soup | （效果未知） |
| Pumpkin Candle | Gain [energy] at the start of each turn. Extinguishes at the start of Act 3. |
| Seal of Gold | At the start of your turn, spend 5 Gold to gain [energy]. |
| Storybook | Upon pickup, add 1 Brightest Flame to your Deck. |
| Toasty Mittens | At the start of your turn, Exhaust the top card of your Draw Pile and gain 1 Strength. |
| Toy Box | Upon pickup, obtain 4 Wax Relics. Every 3 combats, your left-most Wax Relic will melt away. |
| Very Hot Cocoa | Start each combat with an additional 4. |
| Yummy Cookie | Upon pickup, Upgrade 4 cards. |

### 2.16 降级/古代遗物（Ancient/Lowered Tier Relics）

> 带 `???` 后缀的为原版遗物的降级版本，数值低于原版。

| 遗物名称 | 效果 |
|----------|------|
| Anchor??? | Start each combat with 4 Block. |
| Big Mushroom | Upon pickup, raise your Max HP by 20. At the start of each combat, draw 2 fewer cards. |
| Bing Bong | Whenever you add a card to your Deck, add one additional copy. |
| Blood Vial??? | At the start of each combat, heal 1 HP. |
| Bone Tea | At the start of the next combat, Upgrade your starting hand. |
| Byrdpip | Upon pickup, gain the card Byrd Swoop. A Byrdpip will accompany you in battles. |
| Darkstone Periapt | Whenever you obtain a Curse, raise your Max HP by 6. |
| Daughter of the Wind | Whenever you play an Attack, gain 1 Block. |
| Dream Catcher | Whenever you Rest, you may add a card to your Deck. |
| Ember Tea | At the start of the next 5 combats, gain 2 Strength. |
| Forgotten Soul | Whenever you Exhaust a card, deal 1 damage to a random enemy. |
| Fragrant Mushroom | Upon pickup, lose 15 HP and Upgrade 3 random cards. |
| Fresnel Lens | Whenever you add a card that gains Block to your Deck, Enchant it with Nimble 2. |
| Hand Drill | Whenever you break an enemy's Block, apply 2 Vulnerable. |
| Happy Flower??? | Every 5 turns, gain [energy]. |
| History Course | At the start of your turn, play a copy of your last played Attack or Skill. |
| Lee's Waffle??? | Upon pickup, heal 10% of your HP. |
| Lost Wisp | Whenever you play a Power, deal 8 damage to ALL enemies. |
| Mango??? | Upon pickup, raise your Max HP by 3. |
| Maw Bank | Whenever you climb a floor, gain 12 Gold. No longer works when you spend any Gold at the shop. |
| Mr. Struggles | At the start of your turn, deal damage equal to the turn number to ALL enemies. |
| Orichalcum??? | If you end your turn without Block, gain 3 Block. |
| Pollinous Core | Every 4 turns, draw 2 additional cards. |
| Royal Poison | At the start of each combat, lose 4 HP. |
| Snecko Eye??? | （效果未知） |
| Strike Dummy??? | Cards containing "Strike" deal 1 additional damage. |
| Sword of Jade | Start each combat with 3 Strength. |
| Sword of Stone | Transforms into a powerful Relic after defeating 5 Elites. |
| Tea of Discourtesy | At the start of the next combat, shuffle 2 Dazed into your Draw Pile. |
| The Boot | Whenever you would deal 4 or less unblocked attack damage, increase it to 5. |
| The Chosen Cheese | At the end of combat, gain 1 Max HP. |
| The Merchant's Rug??? | Poor imitation. Does nothing. |
| Venerable Tea Set??? | Whenever you enter a Rest Site, start the next combat with an additional [energy]. |
| Wongo Customer Appreciation Badge | Does nothing. |
| Wongo's Mystery Ticket | Receive 3 random Relics after 5 combats. |
| Circlet | It's a circlet. A curious relic which appears when there's a problem within the Spire or there are no more relics to discover. |

---

## 三、怪物（Enemies）

> 数据来源：slaythespire2.gg/enemies
>
> 数据版本：Beta v0.102，最后更新 2026.4.3
>
> HP 格式：普通值（高进阶值）。跨场景怪物在对应场景下各列出一次，以 `*` 标注。
>
> 类型：Boss / Elite / Normal / Weak / Event

### 3.1 Glory

#### Boss

| 名称 | 原名 | HP | 固有能力 | 招式 |
|------|------|----|----------|------|
| 门之制造者 | Doormaker | 489-489(512-512) | — | Dramatic Open, Hunger, Scrutiny, Grasp |
| 猫头鹰判官 | Owl Magistrate | 234-234(243-243) | — | Magistrate Scrutiny, Peck Assault, Judicial Flight, Verdict |
| 女王 | Queen | 400-400(419-419) | — | Puppet Strings, You're Mine, Burn Bright for Me, Off with Your Head, Execution, Enrage |
| 试验体 #C{Count} | Test Subject #C{Count} | — | Enrage×2, Adaptable×1 | Respawn, Bite, Skull Bash, Multi-Claw, Lacerate, Big Pounce+1 |

#### Elite

| 名称 | 原名 | HP | 固有能力 | 招式 |
|------|------|----|----------|------|
| 连枷骑士 | Flail Knight | 101-101(108-108) | — | War Chant, Flail, Ram |
| 机甲骑士 | Mecha Knight | 300-300(320-320) | Artifact×3 | Charge, Flamethrower, Windup, Heavy Cleave |
| 灵魂枢纽 | Soul Nexus | 234-234(254-254) | — | Soul Burn, Maelstrom, Drain Life |
| 幽灵骑士 | Spectral Knight | 93-93(97-97) | — | Hex, Soul Slash, Soul Flame |

#### Normal

| 名称 | 原名 | HP | 固有能力 | 招式 |
|------|------|----|----------|------|
| 斧头机器人 | Axebot | 40-44(42-46) | Stock×2 | Boot Up, The One-Two, Sharpen, Hammer Uppercut |
| 立方构造体* | Cubex Construct | 65-65(70-70) | Artifact×1 | Charge Up, Repeater, Repeater Move 2, Expel Blast, Submerge |
| 制造者 | Fabricator | 150-150(155-155) | — | Fabricate, Fabricating Strike, Disintegrate |
| 青蛙骑士 | Frog Knight | 191-191(199-199) | Plating×15 | For the Queen, Strike Down Evil, Tongue Lash, Beetle Charge |
| 球形头 | Globe Head | 148-148(158-158) | Galvanic×6 | Thunder Strike, Shocking Slap, Galvanic Burst |
| 法术骑士 | Magi Knight | 82-82(89-89) | — | First Power Shield, Dampen, Prep, Magic Bomb, Ram |
| 拳击构造体* | Punch Construct | 55-55(60-60) | Artifact×1 | READY, Strong Punch, Fast Punch |
| 史莱姆狂战士 | Slimed Berserker | 266-266(276-276) | — | Vomit Ichor, Leeching Hug, Smother, Furious Pummeling |
| 被遗忘者 | The Forgotten | 106-106(111-111) | Possess Speed×1 | Miasma, Dread |
| 迷失者 | The Lost | 93-93(99-99) | Possess Strength×1 | Debilitating Smog, Eye Lasers |
| 火炬头融合体 | Torch Head Amalgam | 199-199(211-211) | Minion×1 | Tackle, Tackle, Beam, Tackle, Tackle |

#### Weak

| 名称 | 原名 | HP | 固有能力 | 招式 |
|------|------|----|----------|------|
| 虔诚雕塑师 | Devoted Sculptor | 162-162(172-172) | — | Forbidden Incantation, Savage |
| 活盾 | Living Shield | 55-55(65-65) | Rampart×25 | Shield Slam, Smash |
| 咬噬卷轴 | Scroll of Biting | 31-38(32-39) | Paper Cuts×2 | Chomp, Chew, More Teeth |
| 炮塔操作员 | Turret Operator | 41-41(51-51) | — | Unload Move 1, Unload Move 2, Reload |

### 3.2 Hive

#### Boss

| 名称 | 原名 | HP | 固有能力 | 招式 |
|------|------|----|----------|------|
| 粉碎者 | Crusher | 209-209(219-219) | Back Attack×1, Crab Rage×1 | Thrash, Enlarging Strike, Bug Sting, Adapt, Guarded Strike |
| 知识恶魔 | Knowledge Demon | 379-379(399-399) | — | Curse of Knowledge, Slap, Knowledge Overwhelming, Ponder |
| 火箭 | Rocket | 199-199(209-209) | Back Attack×1, Crab Rage×1, Surrounded×1 | Targeting Reticle, Precision Beam, Charge Up, Laser, Recharge |
| 贪噬者 | The Insatiable | 321-321(341-341) | — | Liquify Ground, Thrash Move 1, Thrash Move 2, Lunging Bite, Salivate |

#### Elite

| 名称 | 原名 | HP | 固有能力 | 招式 |
|------|------|----|----------|------|
| 千足虫 | Decimillipede | 40-46(46-52) | Reattach×25 | Writhe, Bulk, Constrict, Dead, Reattach |
| 昆虫法师 | Entomancer | 145-145(155-155) | Personal Hive×2 | Pheromone Spit, Beeeees!, Spear! |
| 寄生棱镜 | Infested Prism | 200-200(215-215) | Vital Spark×1 | Jab, Radiate, Whirlwind, Pulsate |

#### Normal

| 名称 | 原名 | HP | 固有能力 | 招式 |
|------|------|----|----------|------|
| 碗虫(丝) | Bowlbug (Silk) | 40-43(41-44) | — | Thrash, Toxic Spit |
| 咬合草 | Chomper | 60-64(63-67) | Artifact×2 | Clamp, Screech |
| 猎杀者 | Hunter Killer | 121-121(126-126) | — | Tenderizing Goop, Bite, Puncture |
| 虱祖 | Louse Progenitor | 134-136(138-141) | Curl Up×14 | Web Cannon, Pounce, Curl and Grow |
| 螨虫 | Myte | 61-67(64-69) | — | Toxic Cornucopia, Bite, Suck |
| 产卵器 | Ovicopter | 124-130(126-132) | — | Lay Eggs, Smash, Tenderizer, Nutritional Paste |
| 沉眠甲虫 | Slumbering Beetle | 86-86(89-89) | Plating×15, Slumber×3 | Snore, Roll Out |
| 刺蟾 | Spiny Toad | 116-119(121-124) | — | Protruding Spines, Spike Explosion, Tongue Lash |
| 暗影者 | The Obscura | 123-123(129-129) | — | Illusion, Piercing Gaze, Wail, Hardening Strike |
| 硬蛋 | Tough Egg | 14-18(15-19) | — | HATCH, Nibble |

#### Weak

| 名称 | 原名 | HP | 固有能力 | 招式 |
|------|------|----|----------|------|
| 碗虫(卵) | Bowlbug (Egg) | 21-22(23-24) | — | Bite |
| 碗虫(蜜) | Bowlbug (Nectar) | 35-38(36-39) | — | Thrash, Buff, Thrash |
| 碗虫(石) | Bowlbug (Rock) | 45-48(46-49) | Imbalanced×1 | Headbutt, Dizzy |
| 外骨骼 | Exoskeleton | 24-28(25-29) | Hard to Kill×9 | Skitter, Mandibles, Enrage |
| 飞贼蛙 | Thieving Hopper | 79-79(84-84) | Escape Artist×5 | Thievery, Nab, Hat Trick, Flutter, Escape |
| 穿洞虫 | Tunneler | 87-87(92-92) | — | Bite, Burrow, Below Move 1, Dizzy |

### 3.3 Overgrowth

#### Boss

| 名称 | 原名 | HP | 固有能力 | 招式 |
|------|------|----|----------|------|
| 仪式巨兽 | Ceremonial Beast | 252-252(262-262) | — | Stamp, Plow, Stun, Beast Cry, Stomp, Crush |
| 血亲追随者 | Kin Follower | 58-59(62-63) | Minion×1 | Quick Slash, Boomerang, Power Dance |
| 血亲祭司 | Kin Priest | 190-190(199-199) | — | Orb of Frailty, Orb of Weakness, Soul Beam, Dark Ritual |
| 幽影 | Vantom | 173-173(183-183) | Slippery×9 | Ink Blot, Inky Lance, Dismember, Prepare |

#### Elite

| 名称 | 原名 | HP | 固有能力 | 招式 |
|------|------|----|----------|------|
| 远古偶像 | Bygone Effigy | 127-127(132-132) | Slow×1 | Initial Sleep, Wake, Slashes |
| 鸟龙 | Byrdonis | 91-94(99-99) | Territorial×1 | Peck, Swoop |
| 蛙寄生物 | Phrog Parasite | 61-64(66-68) | Infested×4 | Infect, Lash |

#### Normal

| 名称 | 原名 | HP | 固有能力 | 招式 |
|------|------|----|----------|------|
| 刺客掠袭者 | Assassin Raider | 18-23(19-24) | — | Killshot |
| 斧手掠袭者 | Axe Raider | 20-22(21-23) | — | Swing 1, Swing 2, Big Swing |
| 残暴掠袭者 | Brute Raider | 30-33(31-34) | — | Beat, Roar |
| 弩手掠袭者 | Crossbow Raider | 18-21(19-22) | — | Fire!, Reload |
| 立方构造体* | Cubex Construct | 65-65(70-70) | Artifact×1 | Charge Up, Repeater, Repeater Move 2, Expel Blast, Submerge |
| 长牙眼球 | Eye With Teeth | 6-6 | Illusion×1 | Distract |
| 蝇伞菌 | Flyconid | 47-49(51-53) | — | Vulnerable Spores, Frail Spores, Smash |
| 雾魔 | Fogmog | 74-74(78-78) | — | Illusion, Swipe, Swipe Random, Headbutt |
| 墨滴 | Inklet | 11-17(12-18) | Slippery×1 | Jab, Whirlwind, Piercing Gaze |
| 撕咬兽 | Mawler | 72-72(76-76) | — | Rip and Tear, Roar, Claw |
| 缩小甲虫 | Shrinker Beetle | 38-40(40-42) | — | Shrinker, Chomp, Stomp |
| 蛇行绞杀者 | Slithering Strangler | 53-55(54-56) | — | Constrict, Thwack, Lash |
| 啮果虫 | Snapping Jaxfruit | 31-33(34-36) | — | Energy Orb |
| 追踪掠袭者 | Tracker Raider | 21-25(22-26) | — | Track, Unleash the Hounds |
| 藤蔓行者 | Vine Shambler | 61-61(64-64) | — | Grasping Vines, Swipe, Chomp |

#### Weak

| 名称 | 原名 | HP | 固有能力 | 招式 |
|------|------|----|----------|------|
| 绒毛蠕虫 | Fuzzy Wurm Crawler | 55-57(58-59) | — | First Acid Goop, Acid Goop, Inhale |
| 叶史莱姆(中) | Leaf Slime (M) | 32-35(33-36) | — | Clump Shot, Sticky Shot |
| 叶史莱姆(小) | Leaf Slime (S) | 11-15(12-16) | — | Butt, Goop |
| 啮咬虫 | Nibbit | 42-46(44-48) | — | Butt, Slice, Hiss |
| 枝条史莱姆(中) | Twig Slime (M) | 26-28(27-29) | — | Clump Shot, Sticky Shot |
| 枝条史莱姆(小) | Twig Slime (S) | 7-11(8-12) | — | Butt |
| 蠕动虫 | Wriggler | 17-21(18-22) | — | Nasty Bite, Wriggle, Spawned |

### 3.4 Underdocks

#### Boss

| 名称 | 原名 | HP | 固有能力 | 招式 |
|------|------|----|----------|------|
| 拉加林女族长 | Lagavulin Matriarch | 222-222(233-233) | Plating×12, Asleep×3 | Sleep, Slash, Slash, Disembowel, Soul Siphon |
| 灵魂鱼 | Soul Fysh | 211-211(221-221) | — | Beckon, De-Gas, Gaze, Fade, Scream |
| 瀑布巨人 | Waterfall Giant | 240-240(250-250) | — | Pressurize, Stomp, Ram, Siphon, Pressure Gun, Pressure Up+2 |

#### Elite

| 名称 | 原名 | HP | 固有能力 | 招式 |
|------|------|----|----------|------|
| 幻影园丁 | Phantasmal Gardener | 26-31(27-32) | Skittish×6 | Bite, Lash, Flail, Enlarge |
| 潜伏群落 | Skulking Colony | 74-74(79-79) | Hardened Shell×20 | Smash, Zoom, Inertia, Piercing Stabs |
| 恐惧鳗 | Terror Eel | 140-140(150-150) | Shriek×70 | Crash, Thrashmove, Stun, Terror |

#### Normal

| 名称 | 原名 | HP | 固有能力 | 招式 |
|------|------|----|----------|------|
| 钙化邪教徒 | Calcified Cultist | 38-41(39-42) | — | Incantation, Dark Strike |
| 尸蛞蝓 | Corpse Slug | 25-27(27-29) | Ravenous×4 | Whip Slap, Glomp, Goop |
| 湿气邪教徒 | Damp Cultist | 51-53(52-54) | — | Incantation, Dark Strike |
| 肥胖哥布林 | Fat Gremlin | 13-17(14-18) | — | Spawned, Flee |
| 化石追踪者 | Fossil Stalker | 51-53(54-56) | Suck×3 | Tackle, Latch, Lash |
| 毒气弹 | Gas Bomb | 7-7(8-8) | Minion×1 | Explode |
| 哥布林雇佣兵 | Gremlin Merc | 47-49(51-53) | Surprise×1 | Gimme, Double Smash, Hehe |
| 幽灵船 | Haunted Ship | 63-63(67-67) | — | Ramming Speed, Swipe, Stomp, Haunt |
| 活雾 | Living Fog | 80-80(82-82) | — | Advanced Gas, Bloat, Super Gas Blast |
| 拳击构造体* | Punch Construct | 55-55(60-60) | Artifact×1 | READY, Strong Punch, Fast Punch |
| 排污蛤 | Sewer Clam | 56-56(58-58) | Plating×8 | Pressurize, Jet |
| 双尾鼠 | Two-Tailed Rat | 17-21(18-22) | — | Scratch, Disease Bite, Screech, Call for Backup |

#### Weak

| 名称 | 原名 | HP | 固有能力 | 招式 |
|------|------|----|----------|------|
| 海朋克 | Seapunk | 44-46(47-49) | — | Sea Kick, Spinning Kick, Bubble Burp |
| 污泥纺丝者 | Sludge Spinner | 37-39(41-42) | — | Oil Spray, Slam, Rage |
| 潜行哥布林 | Sneaky Gremlin | 10-14(11-15) | — | Spawned, Tackle |
| 蝌蚪 | Toadpole | 21-25(22-26) | — | Spike Spit, Whirl, Spiken |

### 3.5 特殊/事件

| 名称 | 原名 | HP | 固有能力 | 招式 |
|------|------|----|----------|------|
| 商人 | The Merchant | 165-165(175-175) | — | Swipe, Spew Coins, Throw Relic, Enrage |

### 3.6 跨场景怪物说明

| 原名 | 出现场景 |
|------|----------|
| Cubex Construct | Glory, Overgrowth |
| Punch Construct | Glory, Underdocks |
